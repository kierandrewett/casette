...
export const Player = ({
    ...
    // Seed the store so CaptionsMenu can render the AUTO badge without
    // prop-drilling captions. Has to be in an effect — calling the setter
    // during render with a freshly-allocated Set triggers a store update
    // every render, which re-renders subscribers, which re-runs this line,
    // which is "Maximum update depth exceeded".
    useEffect(() => {
        const autoLabels = new Set(captions.filter((c) => c.isAutoGenerated).map((c) => c.label));
        setAutoGeneratedCaptionLabels(autoLabels);
    }, [captions, setAutoGeneratedCaptionLabels]);
