...
export const Player = ({
    ...
    // Seed the auto-caption-label store once on mount. Captions don't change
    // for the lifetime of a watch page, so depending on the array reference
    // would just risk re-firing the effect each render with a fresh Set,
    // which used to cascade into "Maximum update depth exceeded" via the
    // CaptionsMenu store subscriber. Direct .getState() avoids subscribing
    // this component to the setter.
    useEffect(() => {
        const autoLabels = new Set(captions.filter((c) => c.isAutoGenerated).map((c) => c.label));
        usePlayerStore.getState().setAutoGeneratedCaptionLabels(autoLabels);
